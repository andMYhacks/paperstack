...
def get_claude_client(token: str) -> ClaudeClient:
    return anthropic.Anthropic(api_key=token)


def summarize_abstract_with_claude(client: ClaudeClient, abstract: str) -> str:
    response = client.messages.create(
        model="claude-3-haiku-20240307",
        max_tokens=100,
        temperature=0.5,
        system=SUMMARIZE_ABSTRACT_PROMPT,
        messages=[
...
def get_focus_label_from_abstract(client: ClaudeClient, abstract: str) -> Focus | None:
...
def get_attack_type_from_abstract(client: ClaudeClient, abstract: str) -> AttackType | None:
